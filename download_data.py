...
if __name__ == '__main__':
    ...
    if not os.path.exists(local_filename):
        print('url: ', "%s/%s.gz" % (MNIST_URL, MNIST_FLOAT_TRAIN), local_filename+'.gz')
    ...
    for split in splits:
        ...
        with open(local_filename, 'rb') as f:
            mnist_binarized.append((np.array([list(map(int, line.split())) for line in f.readlines()]).astype('float32'), None))
